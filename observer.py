...
async def observer(encryptionKey, game_id, game_path):
    print(game_path)
    observer_file = f'''rem NIX Spectate
...
if exist "%RADS_PATH%\Game" (
    ...
    if exist "League of Legends.exe" (
        @start "" "League of Legends.exe" "spectator spectator.kr.lol.riotgames.com:80 {encryptionKey} {game_id} KR" "-UseRads" "-Locale=!locale!" "-GameBaseDir=.."
        goto :eof
    )
)
...
:eof
    '''
    f = open(f'NIX_OBSERVER_{game_id}.bat', 'w')
    print(f'ob {game_id}')
    f.write(observer_file)
    print('완료')
    f.close
    return
