...
API_Key = data['MMR_API_KEY']
URL = 'https://kr.whatismymmr.com/api/v1/summoner?name={}&apiKey={}'

def get_summoner_data(search, game_type):
    print(f'{Time()}MMR_API.py_Game_Type : {game_type}\n')
    start = time.time()
    url = URL.format(search, API_Key)
    try:
        ...
        if error:
            ...
            print(f'{Time()}MMR_API.py_Error : {error_msg}\n')
        ...
        print(f'{Time()}MMR_API.py_time : {time.time() - start}\n')
...
def Rank(search='hide on bush'):
    with ThreadPoolExecutor() as executor:
        return executor.submit(get_summoner_data, search, 'ranked').result()

def Normal(search='hide on bush'):
    with ThreadPoolExecutor() as executor:
        return executor.submit(get_summoner_data, search, 'normal').result()

def ARAM(search='hide on bush'):
    with ThreadPoolExecutor() as executor:
        return executor.submit(get_summoner_data, search, 'ARAM').result()